...
\begin{document}
    \begin{frame}[plain]
        % Optionally: specify up to two logos for the title page
        \settitlelogo[1]{logos/liacs}
        \settitlelogo[2]{logos/ludev}
        \titlepage
    \end{frame}
    ...
    % Note: to work around limitations, even if no subtitle is provided to a sideframe, the (empty) second pair '{...}' must be specified
    \begin{sideframe}{Default Side Frame}{}
        The \texttt{sideframe} environment offers decorative sides to be placed. The following options can be supplied:

        \begin{itemize}
            \item \texttt{side}: left or right
            \item \texttt{size}: small or large
            \item \texttt{color}: blue or darkblue
        \end{itemize}

        By default, the environment applies the styling of this slide (\texttt{left}, \texttt{small}, \texttt{blue}).
        To see the other options in action, checkout the next slide!

    \end{sideframe}

    \begin{sideframe}[side=right,size=large,color=darkblue]{Another Slide Frame}{With a Long Subtitle to Show Wrapping}
        Amazing!
    \end{sideframe}
    ...
    \begin{frame}{Two Columns}{List Examples, a Figure, and a Subtitle}
        \begin{columns}[onlytextwidth]%
            ...
            \begin{column}{.5\textwidth}%
                \begin{figure}
                    \centering
                    \includegraphics[width=0.75\linewidth]{graphics/components/corner-logo.png}
...
sayHi('Jort')
    ...
    \begin{frame}[plain]
        \Closure[background=true,circles=true]{Thank you}{Questions?}
    \end{frame}
